//! CallPilot bundled llama.cpp sidecar.
//!
//! Runs a locally downloaded GGUF model for meeting summarization. The parent
//! Tauri app spawns this binary and communicates over JSON lines on
//! stdin/stdout - stdout is reserved for protocol messages, all diagnostics go
//! to stderr. The model is kept loaded across requests and unloaded when the
//! parent exits or an idle timeout elapses.
...
fn calculate_gpu_layers(
    ...
    // Estimate KV cache size: 7B+ models ~256MB per 1k context, smaller ~128MB.
    ...
    eprintln!("VRAM: {:.2} GB available, {:.2} GB safe", vram_gb, safe_vram);
    eprintln!("Model weights: {:.2} GB, KV cache: {:.2} GB", file_size_gb, total_kv_gb);
    ...
    if layers < model_layers {
        eprintln!("Memory constrained. Offloading {}/{} layers", layers, model_layers);
    } else {
...
/// Default GPU layer count with smart detection.
...
impl ModelState {
    ...
    fn load_model_if_needed(&mut self, model_path: PathBuf, context_size: u32) -> Result<()> {
        // Reuse the already-loaded model when nothing changed.
    ...
    fn generate(
        ...
        // Conservative thread count so the UI thread is never starved.
        ...
        eprintln!(
            "Generation stats: prompt={} out={} total={:.2}s speed={:.2} tok/s",
            prompt_tokens,
            output_tokens,
...
fn main() -> Result<()> {
    // Idle timeout can be overridden via LLAMA_IDLE_TIMEOUT (seconds).

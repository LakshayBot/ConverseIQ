...
pub async fn initialize_fresh_database(app: AppHandle) -> Result<(), String> {
    ...
    // CallPilot no longer ships a built-in summary model on the desktop agent —
    // the .NET Gateway + configured provider handles summary via the Intelligence Panel.
    let pool = db_manager.pool();

    let default_summary_model = "server-side";

...
/// Stub: was previously `crate::summary::CustomOpenAIConfig` for the Ollama/OpenAI summary path.
/// CallPilot uses the server-side providers from the .NET Gateway, so the desktop agent
/// only persists per-user model configuration without a custom-openai override.

...
pub struct AudioChunk {
    ...
    /// True if this chunk is an interim slice of an in-progress speech segment.
    /// The UI uses this to render the text incrementally while the user is still
    /// speaking (live streaming). False means the VAD has detected the end of
    /// speech (silence) and the chunk contains the final transcript for that turn.
    pub is_partial: bool,
}

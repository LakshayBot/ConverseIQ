...
pub struct AudioPipeline {
    ...
    // Recording state is currently only kept for symmetry with AudioCapture; the
    // pipeline doesn't read it directly. Prefix with underscore to silence the
    // dead-code warning while keeping the field for future use.
    ...
    // LIVE STREAMING: emit an interim partial transcription chunk every N ms while
    // the VAD is still inside a speech region, so the UI shows text incrementally
    // instead of waiting for end-of-speech silence.
    last_interim_emit: std::time::Instant,
    interim_emit_interval_ms: u64,
}
                        ...
                        if let Some((mic_window, sys_window)) = self.ring_buffer.extract_window() {
                            ...
                            match self.vad_processor.process_audio(&mixed_with_gain) {
                                Ok(speech_segments) => {
                                    for segment in speech_segments {
                                        ...
                                        if segment.samples.len() >= 800 {  // Minimum 50ms at 16kHz - matches Parakeet capability
                                            ...
                                            let transcription_chunk = AudioChunk {
                                                ...
                                                is_partial: false, // Final segment from VAD end-of-speech
                                            };
                                    ...
                                    // LIVE STREAMING: while the VAD is still in-speech, also
                                    // emit an interim "partial" chunk every ~800ms. The UI uses
                                    // these to render text incrementally as the user speaks.
                                    if self.vad_processor.is_in_speech() {
                                        let now = std::time::Instant::now();
                                        let elapsed = now.duration_since(self.last_interim_emit).as_millis() as u64;
                                        if elapsed >= self.interim_emit_interval_ms {
                                            self.last_interim_emit = now;
                                            if let Some(interim) = self.vad_processor.interim_snapshot() {
                                                let interim_samples = interim.samples.len();
                                                if interim_samples >= 1600 {
                                                    // ~100ms minimum for an interim (matches Whisper/Parakeet floor)
                                                    info!("📡 Sending interim partial: {:.1}ms, {} samples",
                                                          interim.end_timestamp_ms - interim.start_timestamp_ms,
                                                          interim_samples);
                                                    let interim_chunk = AudioChunk {
                                                        data: interim.samples,
                                                        sample_rate: 16000,
                                                        timestamp: interim.start_timestamp_ms / 1000.0,
                                                        chunk_id: self.chunk_id_counter,
                                                        device_type: DeviceType::Microphone,
                                                        is_partial: true,
                                                    };
                                                    if let Err(e) = self.transcription_sender.send(interim_chunk) {
                                                        warn!("Failed to send interim partial: {}", e);
                                                    } else {
                                                        self.chunk_id_counter += 1;
                                                    }
                                                }
                                        ...
                                        // Reset the interim timer when speech ends so the next
                                        // speech burst starts fresh.
                                        self.last_interim_emit = std::time::Instant::now();
                                    }
                                }
                                Err(e) => {
                                    warn!("⚠️ VAD error: {}", e);
                ...
                for segment in final_segments {
                    ...
                    if segment.samples.len() >= 800 {
                        ...
                        let transcription_chunk = AudioChunk {
                            ...
                            chunk_id: self.chunk_id_counter,
                            device_type: DeviceType::Microphone,
                            is_partial: false, // Final flush segment
                        };

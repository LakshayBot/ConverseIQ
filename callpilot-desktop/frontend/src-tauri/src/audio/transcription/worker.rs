        ...
        TranscriptionEngine::Parakeet(parakeet_engine) => {
            match parakeet_engine.transcribe_audio(speech_samples).await {
                Ok(text) => {
                    let cleaned_text = text.trim().to_string();
                    if cleaned_text.is_empty() {
                        return Ok((String::new(), None, chunk.is_partial));
                    }

                    info!(
                        "Parakeet transcription complete for chunk {} (partial={}): '{}'",
                        chunk.chunk_id, chunk.is_partial, cleaned_text
                    );

                    // Parakeet doesn't provide intrinsic partials, but the pipeline
                    // tags interim chunks with is_partial=true so the UI can stream text.
                    Ok((cleaned_text, None, chunk.is_partial))
                }
                Err(e) => {
                    error!(
        ...
        TranscriptionEngine::Provider(provider) => {
            ...
            match provider.transcribe(speech_samples, language).await {
                Ok(result) => {
                    ...
                    // The pipeline's chunk-level is_partial takes precedence so partial
                    // flags from interim slices aren't lost when the provider doesn't
                    // surface its own streaming state.
                    let final_partial = chunk.is_partial || result.is_partial;
                    Ok((cleaned_text, result.confidence, final_partial))
                }
                Err(e) => {
                    error!(

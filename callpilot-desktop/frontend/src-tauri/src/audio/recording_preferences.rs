...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\callpilot-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("callpilot-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("callpilot-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/callpilot-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("callpilot-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/callpilot-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("callpilot-recordings")
    }

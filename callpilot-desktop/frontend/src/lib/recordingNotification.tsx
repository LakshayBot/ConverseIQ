      ...
      const toastId = toast.info(
        ...
        description: (
          <div className="space-y-3 min-w-[280px]">
            <p className="text-sm font-medium text-[var(--opaline-on-surface)]">
              Inform all participants this meeting is being recorded.
            </p>
            <label className="flex items-center gap-2 text-xs cursor-pointer hover:bg-[var(--opaline-info-soft)] p-2 rounded transition-colors">
              <input
                type="checkbox"
                onChange={(e) => {
                  dontShowAgain = e.target.checked;
                }}
                className="rounded border-[var(--opaline-outline-variant)] text-primary focus:ring-ring focus:ring-2"
              />
              <span className="select-none text-[var(--opaline-on-surface-variant)]">Don't show this again</span>
            </label>
            <button
              onClick={async () => {
                if (dontShowAgain) {
              ...
              className="w-full px-3 py-1.5 bg-[var(--opaline-ink)] text-white text-xs rounded hover:bg-[var(--opaline-ink-hover)] transition-colors font-medium"
            >

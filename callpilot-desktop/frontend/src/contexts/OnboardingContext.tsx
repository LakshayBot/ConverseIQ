...
export function OnboardingProvider({ children }: { children: React.ReactNode }) {
  ...
  const initializeSummaryModelSelection = async (preferredModel: string = selectedSummaryModel) => {
    ...
    setRecommendedSummaryModel(placeholder);
    setSelectedSummaryModel(placeholder);
    setSummaryModelDownloaded(true);
    return {
      status: 'available' as const,
      selectedSummaryModel: placeholder,
  ...
  const performAutoDetection = async () => {
    ...
    if (typeof navigator !== 'undefined' && navigator.platform?.toLowerCase().includes('mac')) {
      const homebrewDbPath = '/usr/local/var/callpilot/meeting_minutes.db';
      try {
  ...
  const completeOnboarding = async () => {
    try {
      ...
      // Mark onboarding as complete (Rust persists state + transcription config).
    ...
    try {
      ...
      if (shouldStartParakeet) {
        ...
        invoke('parakeet_download_model', { modelName: PARAKEET_MODEL })
          .catch((err: unknown) => console.error('[OnboardingContext] Parakeet download failed:', err));
      }

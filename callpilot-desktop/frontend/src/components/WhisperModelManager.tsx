...
import { Check, Flame, HardDrive, Rocket, Target, Zap } from 'lucide-react';
        ...
        <div className="flex items-start justify-between mb-2">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 flex-wrap mb-2">
              ...
              <span className="text-sm text-[var(--opaline-outline)]">•</span>
              <span className="text-sm text-[var(--opaline-outline)]">{getModelTagline(model.name, model.speed, model.accuracy)}</span>
              {isSelected && isAvailable && (
                <motion.span
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
              ...
              {isQuantizedModel(model.name) && (
                <span className={`px-2 py-0.5 rounded-full text-xs ${getModelPerformanceBadge(model.name).color === 'green'
                  ? 'bg-[var(--opaline-success-soft)] text-success'
                  : getModelPerformanceBadge(model.name).color === 'orange'
                    ? 'bg-[var(--opaline-warning-soft)] text-warning'
                    : 'bg-[var(--opaline-surface-container-low)] text-[var(--opaline-on-surface-variant)]'
                  }`}>
            ...
            {/* Model Specs */}
            <div className="flex items-center space-x-4 text-sm text-[var(--opaline-on-surface-variant)] ml-12 mt-1.5">
              <span className="flex items-center space-x-1.5">
                <HardDrive className="h-3.5 w-3.5 text-[var(--opaline-outline)]" />
                <span>{formatFileSize(model.size_mb)}</span>
              </span>
              <span className="flex items-center space-x-1.5">
                <Target className="h-3.5 w-3.5 text-[var(--opaline-outline)]" />
                <span>{model.accuracy} accuracy</span>
              </span>
              <span className="flex items-center space-x-1.5">
                <Zap className="h-3.5 w-3.5 text-[var(--opaline-outline)]" />

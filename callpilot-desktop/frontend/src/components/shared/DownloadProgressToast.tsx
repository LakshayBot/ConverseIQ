...
function DownloadToastContent({
  ...
  return (
    <div className="flex items-center gap-3 w-full max-w-sm bg-[var(--opaline-surface-container-lowest)] rounded-lg shadow-lg border border-[var(--opaline-outline-variant)] p-3 relative">

      {/* Icon */}
      <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${isComplete ? 'bg-[var(--opaline-success-soft)]' : hasError ? 'bg-[var(--opaline-danger-soft)]' : isCancelled ? 'bg-[var(--opaline-surface-container-low)]' : 'bg-[var(--opaline-surface-container-low)]'
        }`}>
        {isComplete ? (
          ...
          <X className="w-4 h-4 text-[var(--opaline-on-surface-variant)]" />
        ) : (
          <ArrowBigDownDash className="size-5 text-[var(--opaline-on-surface-variant)] " />
        )}
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2 mb-1">
          <p className="text-sm font-medium text-[var(--opaline-on-surface)] truncate">
            {download.displayName}
          </p>
        </div>

        {hasError ? (
          <p className="text-xs text-danger">{download.error || 'Download failed'}</p>
        ) : isComplete ? (
          <p className="text-xs text-success">Download complete</p>
        ) : isCancelled ? (
          <p className="text-xs text-[var(--opaline-on-surface-variant)]">Download cancelled</p>
        ) : (
          <>
            {/* Progress bar */}
            <div className="w-full h-1.5 bg-[var(--opaline-surface-container)] rounded-full overflow-hidden mb-1.5">
              <div
                className="h-full bg-[var(--opaline-ink)] rounded-full transition-all duration-300"
                style={{ width: `${download.progress}%` }}
              />
            </div>

            {/* Progress text */}
            <div className="flex items-center justify-between text-xs text-[var(--opaline-outline)]">
              <span>
              ...
              <span className="flex items-center gap-1">
                ...
                <span className="text-[var(--opaline-on-surface)] font-medium">

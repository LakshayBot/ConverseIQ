  ...
  return (
    <div className={`flex items-center space-x-2 ${className}`}>
      ...
      <div className={`w-2 h-2 rounded-full ${
        isActive ? 'bg-success animate-pulse' : 'bg-[var(--opaline-surface-container-high)]'
      }`} title={`${deviceName} - ${isActive ? 'Active' : 'Inactive'}`} />

      {/* Level meter container */}
      <div className={`flex-1 ${sizes.container} relative`}>
        {/* Background */}
        <div className="w-full h-full bg-[var(--opaline-surface-container)] rounded-sm overflow-hidden">
        ...
        <div className="absolute inset-0 flex justify-between items-center px-1 pointer-events-none">
          {/* 25% marker */}
          <div className="w-px h-full bg-[var(--opaline-outline)] opacity-30" style={{ marginLeft: '25%' }} />
          {/* 50% marker */}
          <div className="w-px h-full bg-[var(--opaline-outline)] opacity-30" style={{ marginLeft: '50%' }} />
          {/* 75% marker */}
          <div className="w-px h-full bg-[var(--opaline-outline)] opacity-30" style={{ marginLeft: '75%' }} />
        </div>
      </div>

      {/* Level percentage display */}
      <div className={`${sizes.text} text-[var(--opaline-on-surface-variant)] font-mono min-w-[3rem] text-right`}>
        {rmsPercent}%
      </div>
    </div>
  );
  ...
  return (
    <div className={`flex items-center space-x-1 ${className}`}>
      ...
      <div className={`w-1.5 h-1.5 rounded-full ${
        isActive ? 'bg-success' : 'bg-[var(--opaline-surface-container-high)]'
      }`} />

      {/* Mini meter */}
      <div className="w-8 h-1.5 bg-[var(--opaline-surface-container)] rounded-sm overflow-hidden">
        <div

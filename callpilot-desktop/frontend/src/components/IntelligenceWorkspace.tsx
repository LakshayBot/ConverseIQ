...
// IntelligenceWorkspace - the two-region intelligence surface inside the
// right rail.
//
//   TOP:    active intelligence - the selected signal's detail (type,
//           name, contextual explanation, knowledge sources).
//   BOTTOM: a horizontal carousel of every detected signal (products,
//           competitors, objections, pricing, technical) - select an item
//           and the top region transitions to it.
//
// All data is DERIVED from the panel's real IntelligenceCards (live
// SignalR detections or historical events/recommendations) - no parallel
// state, no fake data. Selection is local UI state: new arrivals extend
// the carousel without stealing the user's choice (fall back to the
// first item only when the selection disappears). In live mode, freshly
// arrived items get a brief dot so they are noticed without focus theft.
...
export const TYPE_META: Record<IntelligenceCard['type'], { icon: React.ReactNode; label: string }> = {
  ...
  buying_signal:       { icon: <ThumbsUp className="h-3.5 w-3.5" />, label: 'Buying signal' },
...
export const IntelligenceWorkspace: React.FC<IntelligenceWorkspaceProps> = ({ cards, mode }) => {
  ...
  // "New" indication: briefly dot freshly arrived items in live mode so
  // they are noticed without stealing the user's selection.
  ...
  // Keep the current selection stable while new detections arrive; only
  // fall back to the first item when the selection disappears.
  useEffect(() => {
    if (cards.length === 0) {
      setSelectedId(null);
      return;
    ...
    setSelectedId((prev) => {
      if (prev && cards.some((c) => c.title === prev)) return prev;
      return cards[0].title;
    });
  }, [cards]);

  // Right-edge fade while more items exist off-screen.
  useEffect(() => {
  ...
  const selected = cards.find((c) => c.title === selectedId) ?? cards[0] ?? null;
  const selectedIndex = selected ? cards.findIndex((c) => c.title === selected.title) : -1;

  const selectAndReveal = (title: string, index: number) => {
    setSelectedId(title);
    const items = scrollRef.current?.querySelectorAll<HTMLElement>('[data-signal-item]');
  ...
  const onCarouselKeyDown = (e: React.KeyboardEvent) => {
    if (cards.length < 2 || (e.key !== 'ArrowLeft' && e.key !== 'ArrowRight')) return;
    e.preventDefault();
    const next =
      e.key === 'ArrowRight'
        ? Math.min(selectedIndex + 1, cards.length - 1)
        : Math.max(selectedIndex - 1, 0);
    selectAndReveal(cards[next].title, next);
  };

  if (cards.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-1.5 px-4 text-center">
        <p className="text-body-sm font-medium text-[var(--opaline-on-surface)]">No signals yet</p>
        <p className="max-w-[220px] text-caption">
          Relevant products and conversation signals will appear here as they&apos;re detected.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col gap-3">
      {/* ── TOP: active intelligence (detail view) ───────────────────── */}
      <div className="panel-inset flex min-h-0 flex-1 flex-col overflow-hidden">
            ...
            <div key={selected.title} className="animate-fade-soft flex flex-col gap-2.5">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="status-pill !px-2 !py-0.5">
                  <span
                    className={`pill-dot ${SEVERITY_DOT[selected.severity]}`}
                    aria-hidden
                  />
      ...
      {/* ── BOTTOM: horizontal signal carousel ──────────────────────── */}
      <div className="flex shrink-0 flex-col gap-2">
        <div className="flex items-baseline justify-between">
          <h3 className="text-overline">
            {mode === 'live' ? 'Live signals' : 'Detected signals'}
          </h3>
          <span className="text-data text-[var(--opaline-outline)]">{cards.length}</span>
        </div>

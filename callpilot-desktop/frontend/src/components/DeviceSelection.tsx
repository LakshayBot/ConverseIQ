...
export function DeviceSelection({ selectedDevices, onDeviceChange, disabled = false }: DeviceSelectionProps) {
  ...
  const handleMicDeviceChange = (deviceName: string) => {
    ...
    }).catch((err: unknown) => console.error('Failed to track microphone selection:', err));
  };
  ...
  const handleSystemDeviceChange = (deviceName: string) => {
    ...
    }).catch((err: unknown) => console.error('Failed to track system audio selection:', err));
  };
    ...
    return (
      <div className="p-4 space-y-4">
        <div className="animate-pulse">
          <div className="h-4 bg-[var(--opaline-surface-container)] rounded w-1/3 mb-4"></div>
          <div className="h-10 bg-[var(--opaline-surface-container)] rounded mb-3"></div>
          <div className="h-10 bg-[var(--opaline-surface-container)] rounded"></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-[var(--opaline-on-surface)]">Audio Devices</h4>
        <div className="flex items-center space-x-2">
          ...
          {/*     isMonitoring */}
          {/*       ? 'bg-[var(--opaline-danger-soft)] text-danger hover:bg-red-200' */}
          {/*       : 'bg-[var(--opaline-success-soft)] text-success hover:bg-green-200' */}
          ...
          <button
            ...
            className="h-8 w-8 p-0 inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors hover:bg-[var(--opaline-surface-container-low)] disabled:pointer-events-none disabled:opacity-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--opaline-primary)]"
          >
      ...
      {error && (
        <div className="p-3 text-sm text-danger bg-[var(--opaline-danger-soft)] border border-[var(--opaline-danger-border)] rounded-md">
          {error}
        </div>
      )}
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <Mic className="h-4 w-4 text-[var(--opaline-on-surface-variant)]" />
            <Label htmlFor="mic-selection" className="text-sm font-medium text-[var(--opaline-on-surface-variant)]">
              Microphone
            </Label>
          </div>
          <Select
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-[var(--opaline-outline)]">No microphone devices found</p>
          )}

          {/* Audio Level Meters for Input Devices */}
          {showLevels && inputDevices.length > 0 && (
            <div className="space-y-2 pt-2 border-t border-[var(--opaline-outline-variant)]">
              <p className="text-xs text-[var(--opaline-on-surface-variant)] font-medium">Microphone Levels:</p>
              {inputDevices.map((device) => {
                const levelData = audioLevels.get(device.name);
                return (
                  <div key={`level-${device.name}`} className="space-y-1">
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-[var(--opaline-on-surface-variant)] truncate max-w-[200px]">
                        {device.name}
                      </span>
                      {levelData && (
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <Speaker className="h-4 w-4 text-[var(--opaline-on-surface-variant)]" />
            <Label htmlFor="system-selection" className="text-sm font-medium text-[var(--opaline-on-surface-variant)]">
              System Audio
            </Label>
          </div>
          ...
          {outputDevices.length === 0 && (
            <p className="text-xs text-[var(--opaline-outline)]">No system audio devices found</p>
          )}

          {/* Backend Selection - available on all platforms */}
          {!disabled && (
            <div className="pt-3 border-t border-[var(--opaline-outline-variant)]">
      ...
      {/* Info text */}
      <div className="text-xs text-[var(--opaline-outline)] space-y-1">

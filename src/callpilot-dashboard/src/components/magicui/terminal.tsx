// Source: registry/magicui/terminal.tsx
"use client"
...
// Exported so custom sequence surfaces (e.g. the landing live-call demo)
// can drive the same TypingAnimation/AnimatedSpan machinery.
export { SequenceContext }

const useSequence = () => useContext(SequenceContext)

const ItemIndexContext = createContext<number | null>(null)
export { ItemIndexContext }
...
export const Terminal = ({
  ...
  const content = (
    ...
    >
      <div className="border-border flex flex-row items-center gap-x-2 border-b p-3">
        <div className="flex flex-row gap-x-2">
          <div className="h-2.5 w-2.5 rounded-full bg-[var(--color-dot-red)]"></div>
          <div className="h-2.5 w-2.5 rounded-full bg-[var(--color-dot-yellow)]"></div>
          <div className="h-2.5 w-2.5 rounded-full bg-[var(--color-dot-green)]"></div>
        </div>
        {title && (
          <span className="ml-2 truncate font-mono text-[11px] tracking-tight text-[var(--color-neutral)]">
            {title}
          </span>
        )}
      </div>
      <div className="overflow-auto p-4">{wrappedChildren}</div>
    </div>
  )
